...
function(labels, col = "colors", opacity = 1) {
  if(col == "black") {
    ...
    distinct_labels_cleaned <- distinct(as.data.frame(labels_cleaned))
    
    distinct_labels_cleaned$colors <- c(1:length(distinct_labels_cleaned$labels_cleaned))
    
    labels_cleaned <- inner_join(as.data.frame(labels_cleaned), distinct_labels_cleaned, by = 'labels_cleaned')
